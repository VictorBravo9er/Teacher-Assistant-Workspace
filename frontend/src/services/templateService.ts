...
export const templateService = {
  async fetchTemplates(): Promise<Template[]> {
    return logger.measure('CLASS_SERVICE', 'fetchTemplates', async () => {
      ...
      if (error) {
        logger.error('CLASS_SERVICE', 'Failed to fetch templates', error);
        throw error;
      }
  ...
  async createTemplate(payload: Partial<Template>): Promise<Template> {
    return logger.measure('CLASS_SERVICE', `createTemplate:${payload.name || 'unnamed'}`, async () => {
      ...
      if (error) {
        logger.error('CLASS_SERVICE', 'Failed to create template', error);
        throw error;
      }
      ...
      if (payload.instructions && payload.instructions.length > 0) {
        for (const inst of payload.instructions) {
          ...
          if (instError) {
            logger.error('CLASS_SERVICE', 'Failed to insert template instruction', instError);
            throw instError;
          }
          ...
          if (linkError) {
            logger.error('CLASS_SERVICE', 'Failed to link instruction to template', linkError);
            throw linkError;
          }
      ...
      if (payload.materialsPreset && payload.materialsPreset.length > 0) {
        for (const mat of payload.materialsPreset) {
          ...
          if (matError) {
            logger.error('CLASS_SERVICE', 'Failed to insert template material', matError);
            throw matError;
          }
          ...
          if (linkError) {
            logger.error('CLASS_SERVICE', 'Failed to link material to template', linkError);
            throw linkError;
          }
  ...
  async updateTemplate(id: string, payload: Partial<Template>): Promise<Template> {
    return logger.measure('CLASS_SERVICE', `updateTemplate:${id}`, async () => {
      const { data, error } = await supabase
        .from('templates')
        .update({
          name: payload.name,
      ...
      if (error) {
        logger.error('CLASS_SERVICE', `Failed to update template ${id}`, error);
        throw error;
      }
  ...
  async deleteTemplate(id: string): Promise<void> {
    return logger.measure('CLASS_SERVICE', `deleteTemplate:${id}`, async () => {
      ...
      if (error) {
        logger.error('CLASS_SERVICE', `Failed to delete template ${id}`, error);
        throw error;
      }
    });
  }
